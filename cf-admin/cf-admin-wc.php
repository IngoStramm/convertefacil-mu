...
function cf_fix_set_wc_as_active_plugin( $active_plugins ) {
	...
	$wc_is_active = is_plugin_active( 'active_plugins' );
	$wc_active_option = get_option( 'active_plugins');
	$wc_is_in_options = in_array( $wc_file, $wc_active_option );
	// cf_debug( in_array( $wc_file, $wc_active_option ) );
	if( $wc_is_active && !$wc_is_in_options ) :
